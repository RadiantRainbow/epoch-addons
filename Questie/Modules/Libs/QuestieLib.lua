...
function QuestieLib:GetColoredQuestName(questId, showLevel, showState, blizzLike)
    ...
    if not name or name == "" then
        -- Check for runtime stubs first
        local runtimeStub = QuestiePlayer.currentQuestlog and QuestiePlayer.currentQuestlog[questId]
        if runtimeStub and runtimeStub.__isRuntimeStub and runtimeStub.name then
            name = runtimeStub.name
        else
            -- Fall back to quest log cache
            local QuestLogCache = QuestieLoader:ImportModule("QuestLogCache")
            local ok, q = pcall(QuestLogCache.GetQuest, questId)
            if ok and q and q.title then
                name = q.title
            else
    ...
    if not level or level == nil then
        -- Check for runtime stubs first for accurate level
        local runtimeStub = QuestiePlayer.currentQuestlog and QuestiePlayer.currentQuestlog[questId]
        if runtimeStub and runtimeStub.__isRuntimeStub and runtimeStub.level then
            level = runtimeStub.level
        else
            level = QuestiePlayer.GetPlayerLevel()
        end
    end
    ...
    if showState then
        ...
        elseif isComplete == 0 then
            ...
            if q and q.isComplete == true then
                name = name .. " " .. Questie:Colorize("(" .. l10n("Complete") .. ")", "green")
            end
        end
    end
